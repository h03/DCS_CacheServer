...
public class HbaseOperation {
	...
	// 创建表
	public void hbaseCreateTable(String[] cfs) throws IOException{
	...
	// 删除表
	public void hbaseDeleTable(String tableName) throws IOException {
		if(hbaseAdmin.tableExists(tableName)){
	...
	// 插入一行记录
	public void hbaseInsertRow(String tableName, String userRowkey, String columnFamily, String[] column, String[] value) {
		try{
			if(hbaseAdmin.tableExists(tableName)){
		//	table = new HTable(configuration,tableName);
	...
	public void hbaseBatchPut(String tableName, List<Put> batchList) {
		try{
			if(hbaseAdmin.tableExists(tableName)){
	...
	// 删除一行记录
	public void hbaseDeleteRow(String tableName, String userRowkey) throws IOException {
		if(hbaseAdmin.tableExists(tableName)){
	...
	// 按row key查找一行记录
	public void hbaseSelectByRowKey(String tableName,String userRowKey) throws IOException {
	//	HTable table = new HTable(configuration,tableName);
	...
	// 查询表中所有行
	public void hbaseScaner(String tableName) {
	try {
	// HTable table =new HTable(configuration, tableName);
	Scan s =new Scan();
